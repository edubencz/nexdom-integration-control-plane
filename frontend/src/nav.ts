...
export type Resource = 'overview' | 'workflows' | 'test' | 'logs' | 'loggers' | 'metrics' | 'runtimes' | 'environments' | 'deployments' | 'mi-operations' | 'audit-logs' | 'access-control';
...
export function sidebarItems(scope: Scope, currentResource: Resource | null): SidebarItem[] {
  return (Object.entries(MATRIX) as [Resource, (typeof MATRIX)[Resource]][])
    ...
    .map(([resource]) => ({
      resource,
      label: resource === 'audit-logs' ? 'Audit Logs' : resource === 'mi-operations' ? 'MI Operations' : capitalize(resource),
