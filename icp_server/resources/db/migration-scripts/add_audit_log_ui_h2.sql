...
ALTER TABLE audit_logs ADD COLUMN IF NOT EXISTS event_source VARCHAR(30) DEFAULT 'RUNTIME' NOT NULL;
...
ALTER TABLE permissions ADD CONSTRAINT chk_permission_domain CHECK (permission_domain IN ('Integration-Management','Environment-Management','Observability-Management','Project-Management','User-Management','Workflow-Management','Audit-Management'));
INSERT INTO permissions (permission_id, permission_name, permission_domain, resource_type, action, description)
SELECT RANDOM_UUID(), 'audit_mgt:view', 'Audit-Management', 'audit_logs', 'view', 'View audit logs'
